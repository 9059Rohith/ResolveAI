...
def make_pipeline(mode: str) -> SupportPipeline:
    try:
    ...
    if mode == "llm":
        ...
        return SupportPipeline(LLMClassifier(client), retriever, LLMDrafter(client), "llm")
    return SupportPipeline(LocalClassifier(), retriever, mode="local")
...
def main() -> None:
    ...
    print(json.dumps(make_pipeline(args.mode).run(args.message).model_dump(), indent=2))
